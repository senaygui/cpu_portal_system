class StudentGradeReport < Prawn::Document
  def initialize(students)
    ...
    @students.each_with_index do |stud, index|
      ...
      move_down 180
      text "Student's Name: <u>#{stud.student.first_name.capitalize} #{stud.student.middle_name.capitalize} #{stud.student.last_name.capitalize}</u>           Sex: <u>#{stud.student.gender.capitalize}</u>           Year: <u>#{stud.student.year}</u> ",
      ...
      text "Semester: <u>#{stud.semester}</u>         Dept: <u> #{stud.department.department_name.capitalize} </u>",
           inline_format: true, size: 11.5, font_style: :bold
      move_down 10
      text "Program: <u>Masters</u>          A/C Year: <u>#{stud.academic_calendar.calender_year}</u>        ID NO: <u>#{stud.student.student_id}</u>  ",
           inline_format: true, size: 11.5, font_style: :bold
      move_down 10
  ...
  def header_footer
    repeat :all do
      ...
      bounding_box [bounds.left, bounds.bottom + 40], width: bounds.width do
        ...
        text '++251911803339  PoBox.1697/1250 support@cpucollege.edu.et', size: 11, align: :center
      end
    end
  end
  ...
  def each_data_in_table(data, _index)
    ...
    ] + data.semester_registration.course_registrations.where(enrollment_status: 'enrolled').includes(:student_grade).includes(:course).map.with_index do |course, _index|
      [course.course.course_title, course.course.course_code, course.course.credit_hour,
       course.student_grade.letter_grade, course.student_grade.grade_point]
  ...
  def preview_table(data)
    [
      ...
      ['Cumulative Point'] + get_cumulative(get_previous_total(data.student, data.semester), data.total_credit_hour,
