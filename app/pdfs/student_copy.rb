class StudentCopy < Prawn::Document
  def initialize(students, gc_date)
    ...
    @students.each_with_index do |stud, _index|
      text 'CPU COLLEGE', inline_format: true, size: 12, align: :center, font_style: :bold
      move_down 10
  ...
  def each_stud_in_table(value, semester)
    ...
    ] + value.map.with_index do |cr, _index|
      [cr.course.course_code, cr.course.course_title, cr.course.credit_hour,
       cr.course.student_grades.last.letter_grade, cr.course.student_grades.last.grade_point, '']
    end + [[' ', '', '', '', '', ' '],
           ['SGPA', student_grade.sgpa, student_grade.total_credit_hour, '', student_grade.total_grade_point, ''], semester == 1 ? [' ', ' ', '', '', '', ' '] : ['CGPA', student_grade.cgpa, student_grade.total_credit_hour, '', student_grade.total_grade_point, '']] + get_major_cumulative(value)
  end
