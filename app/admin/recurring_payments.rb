ActiveAdmin.register RecurringPayment do
  ...
  permit_params :penalty, :daily_penalty, :mode_of_payment, :student_full_name, :student_id_number, :student_id, :section_id,
                :department_id, :program_id, :academic_calendar_id, :semester_registration_id, :invoice_number, :total_price, :registration_fee, :late_registration_fee, :invoice_status, :last_updated_by, :created_by, :due_date, :semester, :year, payment_transaction_attributes: %i[id invoice_id payment_method_id account_holder_fullname phone_number account_number transaction_reference finance_approval_status last_updated_by created_by receipt_image], inovice_item_ids: []
  ...
  index do
    ...
    column 'Program', sortable: true do |n|
      link_to n.program.program_name, admin_programs_path(n.program) if n.program.present?
    end
  ...
  filter :department_id, as: :search_select_filter, url: proc { admin_departments_path },
                         ...
                         order_by: 'id_asc'
  filter :program_id, as: :search_select_filter, url: proc {
                                                        admin_programs_path
                                                      }, fields: %i[program_name id], display_name: 'program_name', minimum_input_length: 2, order_by: 'id_asc'
  ...
  form title: proc { |invoice| "Invoice NO ##{invoice.invoice_number}" } do |f|
    f.semantic_errors
    f.inputs 'payment transaction',
             for: [:payment_transaction, f.object.payment_transaction || PaymentTransaction.new] do |a|
      ...
      if params[:page_name] == 'payment_transaction'
        a.input :payment_method_id, as: :search_select, url: admin_payment_methods_path,
                                    fields: %i[bank_name id], display_name: 'bank_name', minimum_input_length: 2,
                                    order_by: 'id_asc'
      ...
      a.input :finance_approval_status, as: :select,
                                        collection: ['pending', 'approved', 're-submit', 'denied', 'under submitted'], include_blank: false
    ...
    f.inputs 'invoice status' do
      f.input :invoice_status, as: :select,
                               collection: ['pending', 'approved', 're-submit', 'denied', 'under submitted'], include_blank: false
    end
  ...
  action_item :edit, only: :show, priority: 1 do
    if ((current_admin_user.role == 'admin') || (current_admin_user.role == 'finance head')) && !recurring_payment.payment_transaction.present?
      link_to 'Add Payment Information',
              edit_admin_recurring_payment_path(recurring_payment.id, page_name: 'payment_transaction')
    elsif ((current_admin_user.role == 'admin') || (current_admin_user.role == 'finance head')) && recurring_payment.payment_transaction.present?
      link_to 'Edit Payment Information',
              edit_admin_recurring_payment_path(recurring_payment.id, page_name: 'payment_transaction')
    end
  end
  ...
  action_item :delete, only: :show, priority: 1 do
    if (current_admin_user.role == 'admin') || (current_admin_user.role == 'finance head')
      link_to 'Delete Payment', admin_recurring_payment_path(recurring_payment), method: :delete,
                                                                                 'data-confirm': 'Invoice will be deleted forever. Are You Sure?'
    end
  end
  action_item :delete, only: :show, priority: 1 do
    if (current_admin_user.role == 'admin') || (current_admin_user.role == 'finance head')
        ...
        panel 'invoice summery' do
          attributes_table_for recurring_payment do
            ...
            row 'registration academic year' do |s|
              link_to s.semester_registration.academic_calendar.calender_year,
                      admin_semester_registration_path(s.semester_registration.id)
            end
            ...
            if recurring_payment.penalty > 0
              number_row :penalty, as: :currency, unit: 'ETB', format: '%n %u', delimiter: ',',
                                   precision: 2
            end
            if recurring_payment.daily_penalty > 0
              number_row :daily_penalty, as: :currency, unit: 'ETB', format: '%n %u', delimiter: ',',
                                         precision: 2
            end
        ...
        panel 'Invoice Item Information' do
          table_for recurring_payment.invoice_items do
            ...
            column 'Course module' do |pr|
              link_to pr.course_registration.course.course_module.module_code,
                      admin_course_module_path(pr.course_registration.course.course_module.id)
            end
